...
import { Copy, Download, Save, Trash2, ArrowLeft, Check, AlertTriangle } from "lucide-react";
...
export function PostEditor({ postJson, citationJson }: { postJson: string; citationJson: string }) {
  ...
  function updateBlock(key: Block, value: string) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-base">🇺🇸 Caption EN (o que vai postado)</CardTitle>
          ...
          <CardContent className="space-y-3">
            <BlockField
              label="Hook"
              value={out.hook || ""}
              onChange={(v) => updateBlock("hook", v)}
              onCopy={() => copyBlock(out.hook || "", "Hook")}
            />
            <BlockField
              label="Insight"
              value={out.insight || ""}
              onChange={(v) => updateBlock("insight", v)}
            ...
            <BlockField
              ...
              onChange={(v) => updateBlock("reflection", v)}
              onCopy={() => copyBlock(out.reflection || "", "Reflection")}
            />
            <BlockField
              label="CTA"
              value={out.cta || ""}
              onChange={(v) => updateBlock("cta", v)}
            ...
            <BlockField
              label="Alt Text (EN)"
              value={out.altText || ""}
              onChange={(v) => updateBlock("altText", v)}
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">🇧🇷 Tradução PT (referência Samara)</CardTitle>
            <CardDescription className="text-xs">
              Não posta isso — só serve pra Samara entender o que está sendo publicado.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <BlockField
